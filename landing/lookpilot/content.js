(function() {
  ...
  // O card entra na camada de topo do Chrome (popover), não só na página.
  // z-index não chega: a Zara abre a gaveta da composição nessa camada, que
  // fica acima de QUALQUER z-index, e o card ficava por baixo — visível, mas
  // com os cliques a irem parar à gaveta. Era este o "botão morto".
  // Vale para a classe toda (gavetas, modais, widgets de chat), não só Zara.
  // Se o navegador não tiver popover, fica só o appendChild de antes.
  ...
  // Um link com target=_blank dentro do card é um link DA PÁGINA: a Zara (e o
  // resto da Inditex) vigia os scripts de terceiros e engole esse clique —
  // "Ver análise completa" não fazia nada, sem erro nenhum, e a pessoa ficava
  // presa no card. Pedir ao background (chrome.tabs.create) passa por fora da
  // loja. O href fica no <a> para o botão direito → "abrir em nova aba".
  // Se a extensão foi recarregada, esta aba ficou órfã e o pedido falha: aí
  // tenta window.open, que é melhor do que nada.
  function abrirAba(url) {
    try {
      chrome.runtime.sendMessage({ action: 'abrirAba', url }, () => {
  ...
  // Todo link do card que leva para fora (data-lp-abrir) passa por abrirAba.
  // Escuta na fase de captura: a loja pode travar o clique antes de chegar
  // ao próprio link.
  function ligaAbrirAba(card) {
    card.querySelectorAll('a[data-lp-abrir]').forEach((a) => {
  ...
  function injectCard(section) {
    ...
    card.innerHTML = `
      ...
      <div style="padding:12px 14px;">
        ...
        <a href="${buildAnaliseURL(scores, fibers, buy, bv.label, location.href, conf, historia)}" target="_blank" data-lp-abrir style="display:flex;align-items:center;justify-content:center;gap:5px;font-size:12px;font-weight:600;color:#fff;background:#1D1D1F;text-decoration:none;border:none;padding:11px;border-radius:9px;letter-spacing:0.02em;">
  ...
  function injectEmptyCard() {
    ...
    if (report) report.addEventListener('click', () => {
      ...
      abrirAba('mailto:nathy.glima@gmail.com?subject=' + assunto + '&body=' + corpo);
    });
  }
