...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  // O card pede para abrir a análise (ou o email de "loja não funciona").
  // Só estes destinos: o pedido vem de um script que vive dentro de páginas
  // de terceiros, e não deve servir para abrir endereço qualquer.
  if (msg.action === 'abrirAba' && typeof msg.url === 'string' &&
      /^(https:\/\/www\.lookpilotapp\.com\/|http:\/\/localhost:8777\/|mailto:)/.test(msg.url)) {
    // Responde sempre: sem resposta o card lê lastError e abriria a aba DUAS
    // vezes (a do Chrome e a do plano B).
